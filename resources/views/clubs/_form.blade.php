...
<div>
    ...
    <input id="name" name="name" type="text" required maxlength="255" value="{{ old('name', $club->name) }}" class="w-full rounded-md border-2 border-slate-400 focus:border-sky-700 focus:ring-sky-700">
...
<div>
    ...
    <input id="url" name="url" type="url" maxlength="255" value="{{ old('url', $club->url) }}" class="w-full rounded-md border-2 border-slate-400 focus:border-sky-700 focus:ring-sky-700">
    @error('url') <p class="text-sm text-red-700 mt-1">{{ $message }}</p> @enderror
</div>
<div>
    <label for="facebook_url" class="block text-sm font-semibold mb-1">Facebook URL</label>
    <input id="facebook_url" name="facebook_url" type="url" maxlength="2048" value="{{ old('facebook_url', $club->facebook_url) }}" class="w-full rounded-md border-2 border-slate-400 focus:border-sky-700 focus:ring-sky-700">
    @error('facebook_url') <p class="text-sm text-red-700 mt-1">{{ $message }}</p> @enderror
</div>
<div>
    <label for="town" class="block text-sm font-semibold mb-1">Town</label>
    <input id="town" name="town" type="text" maxlength="255" value="{{ old('town', $club->town) }}" class="w-full rounded-md border-2 border-slate-400 focus:border-sky-700 focus:ring-sky-700">
...
<div>
    ...
    <input id="address" name="address" type="text" maxlength="255" value="{{ old('address', $club->address) }}" class="w-full rounded-md border-2 border-slate-400 focus:border-sky-700 focus:ring-sky-700">
...
<div>
    ...
    <input id="phone" name="phone" type="text" maxlength="50" value="{{ old('phone', $club->phone) }}" class="w-full rounded-md border-2 border-slate-400 focus:border-sky-700 focus:ring-sky-700">
...
<div>
    ...
    <textarea id="overview" name="overview" rows="6" maxlength="5000" class="w-full rounded-md border-2 border-slate-400 focus:border-sky-700 focus:ring-sky-700">{{ old('overview', $club->overview) }}</textarea>
...
@if ($withLogo)
    <div>
        ...
        @if ($club->logoUrl())
            ...
            >
                <img src="{{ $club->logoUrl() }}" alt="{{ $club->name }} logo" class="h-16 w-16 object-contain border border-slate-300 rounded bg-white p-1">
            </button>
        @endif
