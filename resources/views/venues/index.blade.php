<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form method="GET" class="bg-white border-2 border-slate-300 rounded-xl p-4 mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-800 mb-1">Club link</label>
                <select name="club_link" class="w-full rounded-md border-2 border-slate-400 focus:border-sky-700 focus:ring-sky-700">
                    <option value="">Any</option>
                    <option value="club" @selected(($filters['club_link'] ?? '') === 'club')>Club waters</option>
                    <option value="independent" @selected(($filters['club_link'] ?? '') === 'independent')>Independent</option>
                </select>
            </div>
