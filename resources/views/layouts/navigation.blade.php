<nav x-data="{ open: false }" class="relative z-50 bg-paper-bright/95 backdrop-blur-sm border-b border-[#d6cfc2]">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 gap-4">
            <div class="flex items-center gap-6 min-w-0">
                <a href="{{ route('home') }}" class="flex items-center gap-2.5 shrink-0">
                    <span class="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-moss text-paper-bright font-bold text-sm tracking-tight shadow-soft">NE</span>
            ...
            <div class="hidden lg:flex lg:items-center lg:gap-3 shrink-0">
                @auth
                    <a href="{{ route('venues.create') }}" class="inline-flex items-center px-3.5 py-2 text-sm font-semibold rounded-lg bg-moss text-white hover:bg-moss-dark whitespace-nowrap">
                        Add Venue
                    </a>
                    <a href="{{ route('sessions.create') }}" class="inline-flex items-center px-3.5 py-2 text-sm font-semibold rounded-lg border border-[#c4bbad] text-ink hover:bg-moss-soft whitespace-nowrap">
                        Log Session
                    </a>
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center max-w-[11rem] px-3 py-2 text-sm font-medium rounded-md text-ink-muted hover:text-ink">
                ...
                @else
                    <a href="{{ route('login') }}" class="inline-flex items-center px-3.5 py-2 text-sm font-semibold rounded-lg border border-moss text-moss-dark hover:bg-moss-soft">Log in</a>
                    <a href="{{ route('register') }}" class="inline-flex items-center px-3.5 py-2 text-sm font-semibold rounded-lg bg-moss text-white hover:bg-moss-dark">Register</a>
                @endauth
            </div>

            <div class="-me-2 flex items-center gap-1 lg:hidden">
                <button @click="open = ! open" class="inline-flex items-center justify-center p-2 rounded-md text-ink-muted hover:bg-moss-soft" aria-label="Toggle navigation">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden lg:hidden border-t border-[#d6cfc2] bg-paper-bright">
        ...
        <div class="pt-4 pb-3 border-t border-[#d6cfc2]">
            @auth
                <div class="px-4 mb-2">
